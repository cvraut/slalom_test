...
def abf(beta, se, W=0.04):
    ...
    denom = sp.special.logsumexp([float(e) for e in lbf])
...
def main(args):
    ...
    if args.weighted_average_r is not None:
        ...
        if len(n_samples) == 1:
            ...
            ld = np.array(ld).T
            df["r"] = np.array([n/d if d != 0 else 0.0 for n,d in zip(np.nansum(n_samples * ld, axis=1) , np.nansum(n_samples * ~np.isnan(ld), axis=1))])
    ...
    if args.dentist_s:
        ...
        df["nlog10p_dentist_s"] = sp.stats.chi2.logsf([float(e) for e in df["t_dentist_s"]],df=1)/-np.log(10)
